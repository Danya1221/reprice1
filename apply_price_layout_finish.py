...
# 1) Apple Watch: Series / SE / Ultra all belong to one publication block and one catalog button.
s, n = re.subn(
...
# 2) iPhone 11-15: keep exact names in rows, but publish them in one message/button.
...
# 3) Samsung: understand supplier's bare Buds / A / S rows even without repeated 'Samsung'.
...
new_samsung = '''def samsung_block(title):
    ...
    # Supplier often omits the word Samsung from every row. These model prefixes
    # are therefore recognized on their own as well.
...
# Reclassify already-saved rows into the new merged blocks.
s = rep(
    s,
    ...
    "saved row reclassification",
)

# New parsed iPhones 11-15 also go to the merged publication block.
s = rep(
    s,
    ...
    "parsed iPhone publication block",
)

# Nice subsections inside the combined Samsung A + S25 message.
...
new_render = '''                last_sim = None
                ...
                for item in sorted(status_items, key=lambda i: (SIM_ORDER.get(i.sim, 99), item_sort(i))):
                    ...
                    row = display_title(item) + " — " + price_text(marked_price(item, settings, overrides), item.currency)
'''
s = rep(s, old_render, new_render, "Samsung render subsections")

p.write_text(s, encoding="utf-8")

# ---- tests ----
reg = Path("tests/test_parser_regressions.py")
t = reg.read_text(encoding="utf-8")nt = rep(
    t,
...
c = rep(
    ...
    "complete Apple Watch expectation",
)
